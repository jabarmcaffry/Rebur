 ...
 *
 * Includes an entity pool: destroyed ids and their component sets are
 * recycled instead of GC'd, so spawn-heavy games (bullets, particles)
...
export class World {
  ...
  // ---- Entity pool ---------------------------------------------------------
  /** Recycled entity ids ready for reuse. */
  private freeIds: number[] = [];
  /** Recycled component-id sets ready for reuse. */
  private freeSets: Set<string>[] = [];

  create(): EntityId {
    let id: number;
    let comps: Set<string>;
    if (this.freeIds.length > 0) {
      id = this.freeIds.pop()!;
      comps = this.freeSets.pop() ?? new Set<string>();
    } else {
      id = this.nextId++;
  ...
  destroy(id: EntityId): void {
    ...
    // Return to pool (cap to bound memory).
    if (this.freeIds.length < POOL_CAP) {
      this.freeIds.push(id);
      this.freeSets.push(comps);
    }
    this.version++;
  }
  ...
  *query(...defs: ComponentDef<unknown>[]): IterableIterator<unknown[]> {
  ...
  /** Diagnostics. */
  poolStats(): { freeIds: number; liveEntities: number } {
    return { freeIds: this.freeIds.length, liveEntities: this.entities.size };
  }
}
