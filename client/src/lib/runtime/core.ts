...
// pointVsObjectSurface is now imported from ./utils/helpers
...
export class GameRuntime {
  ...
  isObjectHeld(objId: string): boolean {
    for (const [, motor] of this.motorState) {
      if (motor.obj.id === objId) return true;
    }
    return false;
  }
  ...
  getHeldObjectSlot(objId: string): { slot: string; offset: Vec3; rotation: Vec3 } | null {
    for (const [slot, motor] of this.motorState) {
      if (motor.obj.id === objId) return { slot, offset: motor.offset, rotation: motor.rotation };
    }
    return null;
  }
  ...
  constructor(snap: GameObject[], scripts: Script[], username: string, avatarColor: string) {
    ...
    // Feet-based player.position: y = where the feet stand. Spawn just above
    // the spawn pad's top surface so the avatar lands cleanly on it.
    ...
    this.player = {
      ...
      spawnPoint,
      up: { x: 0, y: 1, z: 0 },
      // Feet-based: position.y is the bottom of the avatar, center is at +ph.
      collisionRadius: 0.4,
  ...
  private async runScripts() { const api = this.buildApi(0); for (const s of this.scripts) { if (s.error) { this.pushLog(`[${s.name}] ${s.error}`); continue; } if (!s.run) continue; try { const maybe = (s.run as any)(api); if (maybe && typeof maybe.then === "function") maybe.catch((e: any) => this.pushLog(formatScriptErr(e, s.name))); } catch (e: any) { this.pushLog(formatScriptErr(e, s.name)); } } }
  ...
  private applyMotors() {
    ...
    const slotOffsets: Record<string, Vec3> = {
      // Feet-based: y is height above the feet (player.position.y).
  ...
  const p = this.player;
  const pr = p.collisionRadius ?? 0.4;
  const ph = p.collisionHalfHeight ?? 1.12;
  const cy = p.position.y + ph; // feet-based -> center
  ...
  for (const o of this.objectList) {
    ...
    if (o.primitiveType === "sphere") {
      ...
      touching = Math.hypot(p.position.x - o.position.x, cy - o.position.y, p.position.z - o.position.z) < r + pr + 0.05;
    } else {
      touching = Math.abs(p.position.x - o.position.x) < (o.scale.x || 1) * 0.5 + pr &&
                 Math.abs(cy - o.position.y) < (o.scale.y || 1) * 0.5 + ph &&
