...
const COMPLETIONS: CompletionDef[] = [
  ...
  {
    label: "Rebur.Workspace.raycastAll",
    kind: K.Function,
    detail: "Rebur.Workspace.raycastAll(origin, direction, opts?) → RaycastResult[]",
    doc: "Cast a ray and return ALL entities it hits, sorted by distance (nearest first).\n\nSame opts as raycast: maxDistance, ignore, tag.\n\nUseful for penetrating projectiles or \"select all targets in line of sight\":\n\n  const hits = Rebur.Workspace.raycastAll(origin, dir, { maxDistance: 80 });\n  for (const h of hits) h.entity.takeDamage(10);",
    insert: "Rebur.Workspace.raycastAll(\n\t${1:origin},\n\t${2:direction},\n\t{ maxDistance: ${3:50} }\n)",
    snippet: true,
  },
  {
    label: "Rebur.Workspace.overlapSphere",
    kind: K.Function,
    detail: "Rebur.Workspace.overlapSphere(center, radius, opts?) → Entity[]",
    doc: "Return every entity whose bounding box overlaps a sphere.\n\nOpts: { tag?: string; ignore?: Entity[] }\n\nUseful for AOE damage, proximity triggers, and item pickups:\n\n  const nearby = Rebur.Workspace.overlapSphere(player.position, 5, { tag: \"enemy\" });\n  for (const e of nearby) e.takeDamage(30);",
    insert: "Rebur.Workspace.overlapSphere(${1:center}, ${2:5})",
    snippet: true,
  },
  {
    label: "Rebur.Workspace.overlapBox",
    kind: K.Function,
    detail: "Rebur.Workspace.overlapBox(center, halfExtents, opts?) → Entity[]",
    doc: "Return every entity whose bounding box overlaps an axis-aligned box.\n\nhalf­Extents: { x, y, z } — half-size along each axis from the center.\n\nOpts: { tag?: string; ignore?: Entity[] }\n\n  const inZone = Rebur.Workspace.overlapBox(\n    { x: 0, y: 1, z: 0 },\n    { x: 10, y: 2, z: 10 }\n  );",
    insert: "Rebur.Workspace.overlapBox(${1:center}, { x: ${2:5}, y: ${3:2}, z: ${4:5} })",
    snippet: true,
  },
  {
  ...
  // ─── Entity events ────────────────────────────────────────────────────────
  {
    ...
    doc: "Subscribe to an entity event. Returns an unsubscribe function.\n\nBuilt-in events:\n- 'touched'          — (other: Entity) — overlap starts\n- 'untouched'        — (other: Entity) — overlap ends\n- 'clicked'          — (player: PlayerEntity) — 3D viewport click\n- 'destroyed'        — () — entity destroyed\n- 'died'             — () — health reached 0 (fires before autoDestroy)\n- 'interact'         — (player: PlayerEntity) — player pressed E in range\n- 'playerEntered'    — (player: PlayerEntity) — player entered vehicle\n- 'playerExited'     — (player: PlayerEntity) — player exited vehicle\n- 'collisionStarted' — (other, impulse) — physics collision\n- 'collisionEnded'   — (other) — collision ends\n\nCustom events — pair with entity.emit().",
    insert: ".on(\"${1|touched,untouched,clicked,destroyed,died,interact,playerEntered,playerExited,collisionStarted,collisionEnded|}\", (${2:other}) => {\n\t${3}\n})",
    snippet: true,
  },
  {
    label: "entity.off",
  ...
  {
    label: "Rebur.Camera.getForwardRay",
    kind: K.Function,
    detail: "Rebur.Camera.getForwardRay(player) → { origin, direction } | null",
    doc: "Returns the ray from the player's camera in the direction they are looking.\n\nReturns null when camera state is not yet available (pre-first-frame).\n\nUseful for shooters, placement, or any 'what am I pointing at?' query:\n\n  const ray = Rebur.Camera.getForwardRay(player);\n  if (ray) {\n    const hit = Rebur.Workspace.raycast(ray.origin, ray.direction, { maxDistance: 100 });\n  }",
    insert: "Rebur.Camera.getForwardRay(${1:player})",
    snippet: true,
  },
  {
    label: "Rebur.Camera.raycast",
    kind: K.Function,
    detail: "Rebur.Camera.raycast(player, opts?) → { entity, point, normal, distance } | null",
    doc: "Shoots a ray from the player's camera along their look direction and returns the first entity hit.\n\nEquivalent to calling getForwardRay then Rebur.Workspace.raycast. Returns null if nothing is hit.\n\nOptions: { maxDistance?: number; filter?: (entity) => boolean }\n\n  const hit = Rebur.Camera.raycast(player, { maxDistance: 100 });\n  if (hit) hit.entity.takeDamage(25);",
    insert: "Rebur.Camera.raycast(${1:player}, { maxDistance: ${2:100} })",
    snippet: true,
  },
  {
    label: "Rebur.Camera.setForPlayer",
    kind: K.Function,
    detail: "Rebur.Camera.setForPlayer(player, opts) → void",
    doc: "Override camera settings for a single player. Useful for cutscenes or spectator views.\n\nOpts: { mode?, distance?, fov?, position?, lookAt? }\n\n  Rebur.Camera.setForPlayer(player, { mode: 'scripted', position: {x:0,y:20,z:0}, lookAt: entity.position });",
    insert: "Rebur.Camera.setForPlayer(${1:player}, { ${2} })",
    snippet: true,
  },
  {
    label: "Rebur.Camera.setForAll",
    kind: K.Function,
    detail: "Rebur.Camera.setForAll(opts) → void",
    doc: "Override camera settings for every player simultaneously.\n\nOpts: { mode?, distance?, fov?, position?, lookAt? }",
    insert: "Rebur.Camera.setForAll({ ${1} })",
    snippet: true,
  },
  {
    label: "Rebur.Camera.clearForPlayer",
    kind: K.Function,
    detail: "Rebur.Camera.clearForPlayer(player) → void",
    doc: "Remove any per-player camera override, restoring the global camera settings for that player.",
    insert: "Rebur.Camera.clearForPlayer(${1:player})",
    snippet: true,
  },
  ...
  // ─── Game Snippets ────────────────────────────────────────────────────────
  {
    label: "snippet: FPS Shoot",
    kind: K.Snippet,
    detail: "Raycast from camera and deal damage on fire",
    doc: "Shoot a ray from the player's camera on spacebar. Deals damage, shows particles, and logs the hit.\n\nUses Rebur.Camera.raycast, entity.takeDamage, and Rebur.Particles.",
    insert: "// FPS Shoot — raycast from camera on spacebar\nRebur.Input.on(\"press\", (player, key) => {\n\tif (key !== \" \") return;\n\n\tconst hit = Rebur.Camera.raycast(player, { maxDistance: 100 });\n\tif (!hit) return;\n\n\tconst { entity, point } = hit;\n\tentity.takeDamage(${1:25});\n\tRebur.Particles.muzzleFlash(player.position, { x: 0, y: 0, z: -1 });\n\tRebur.Particles.hit(point);\n\tlog(player.username, \"shot\", entity.name, \"→ HP:\", entity.health);\n});",
    snippet: true,
  },
  {
    label: "snippet: AOE Explosion",
    kind: K.Snippet,
    detail: "Damage all entities within a radius with falloff",
    doc: "Reusable explode() function that damages everything within a sphere with distance-based falloff. Wire it to a mine or trigger.\n\nUses Rebur.Workspace.all, entity.takeDamage, Rebur.Particles.explosion, Rebur.Sound.playAt.",
    insert: "// AOE Explosion — damage everything within radius\nfunction explode(position, radius, damage) {\n\tconst targets = Rebur.Workspace.all();\n\tfor (const t of targets) {\n\t\tconst dx = t.position.x - position.x;\n\t\tconst dy = t.position.y - position.y;\n\t\tconst dz = t.position.z - position.z;\n\t\tconst dist = Math.sqrt(dx*dx + dy*dy + dz*dz);\n\t\tif (dist < radius) {\n\t\t\tconst falloff = 1 - dist / radius;\n\t\t\tt.takeDamage(Math.round(damage * falloff));\n\t\t}\n\t}\n\tRebur.Particles.explosion(position, { count: 50, speed: 12, color: \"#ff6600\" });\n\tRebur.Sound.playAt(\"explosion\", position, { maxDistance: 40 });\n}\n\nconst mine = Rebur.Workspace.find(\"${1:Mine}\");\nif (mine) {\n\tmine.on(\"touched\", (other) => {\n\t\tif (other.isPlayer) {\n\t\t\texplode(mine.position, ${2:8}, ${3:80});\n\t\t\tmine.destroy();\n\t\t}\n\t});\n}",
    snippet: true,
  },
  {
    label: "snippet: Item Pickup",
    kind: K.Snippet,
    detail: "E-key interaction that adds item to inventory and destroys the pickup",
    doc: "Enable interaction on an entity so the player can press E to collect it. Adds to inventory, plays particles and sound, then destroys the item.\n\nUses entity.interactionEnabled, entity.on('interact'), player.inventory.add, Rebur.Particles.pickup.",
    insert: "// Item Pickup — press E to collect\nconst ${1:coin} = Rebur.Workspace.find(\"${2:Coin}\");\nif (${1:coin}) {\n\t${1:coin}.interactionEnabled = true;\n\t${1:coin}.interactionHint = \"${3:Press E to pick up}\";\n\n\t${1:coin}.on(\"interact\", (player) => {\n\t\tplayer.inventory.add(\"${2:Coin}\", { count: 1 });\n\t\tRebur.Particles.pickup(${1:coin}.position);\n\t\tRebur.Sound.playForPlayer(player, \"${4:collect}\", { volume: 0.8 });\n\t\t${1:coin}.destroy();\n\t\tlog(player.username, \"picked up a ${2:Coin}\");\n\t});\n}",
    snippet: true,
  },
  {
    label: "snippet: Health Bar HUD",
    kind: K.Snippet,
    detail: "Per-player health bar that updates live on each tick",
    doc: "Shows a coloured health bar in the bottom-left of each player's screen. Updates every tick; color shifts green → yellow → red.\n\nUses player.gui.bar, Rebur.on('playerJoined'), Rebur.on('tick').",
    insert: "// Health Bar HUD — per-player\nRebur.on(\"playerJoined\", (player) => {\n\tplayer.gui.bar(\"hp\", player.health, player.maxHealth, {\n\t\tanchor: \"bl\", x: 20, y: 20,\n\t\twidth: 200, height: 16,\n\t\tcolor: \"#22c55e\", bg: \"#374151\",\n\t});\n\tplayer.gui.text(\"hp-label\", \"HP\", {\n\t\tanchor: \"bl\", x: 20, y: 40,\n\t\tsize: 12, color: \"#ffffff\",\n\t});\n});\n\nRebur.on(\"tick\", () => {\n\tfor (const p of Rebur.Players.all()) {\n\t\tconst pct = p.health / p.maxHealth;\n\t\tconst color = pct > 0.5 ? \"#22c55e\" : pct > 0.25 ? \"#eab308\" : \"#ef4444\";\n\t\tp.gui.bar(\"hp\", p.health, p.maxHealth, { color });\n\t}\n});",
    snippet: true,
  },
  {
    label: "snippet: Death Screen",
    kind: K.Snippet,
    detail: "Show a death overlay and auto-respawn after a delay",
    doc: "Prevents immediate auto-respawn, shows a 'YOU DIED' overlay, then respawns the player after 3 s.\n\nUses player.autoRespawn, player.gui.text, after(), player.respawn.",
    insert: "// Death Screen — overlay then respawn\nRebur.on(\"playerDied\", (player) => {\n\tplayer.autoRespawn = false;\n\n\tplayer.gui.text(\"death\", \"YOU DIED\", {\n\t\tanchor: \"cc\", size: ${1:48}, color: \"#ef4444\",\n\t});\n\tplayer.gui.text(\"death-sub\", \"Respawning in ${2:3} s\u2026\", {\n\t\tanchor: \"cc\", y: 60, size: 18, color: \"#ffffff99\",\n\t});\n\n\tafter(${2:3}, () => {\n\t\tplayer.respawn = true;\n\t\tplayer.health = player.maxHealth;\n\t\tplayer.gui.clear(\"death\");\n\t\tplayer.gui.clear(\"death-sub\");\n\t});\n});",
    snippet: true,
  },
  {
    label: "snippet: Door Tween",
    kind: K.Snippet,
    detail: "Smooth open/close door on E-key with a tween",
    doc: "Toggle a door open and closed when the player presses E nearby. Uses Rebur.Tween for smooth rotation and sound on each state change.\n\nUses entity.interactionEnabled, entity.on('interact'), Rebur.Tween.",
    insert: "// Door Tween — smooth open/close on interact\nconst door = Rebur.Workspace.find(\"${1:Door}\");\nif (door) {\n\tdoor.interactionEnabled = true;\n\tdoor.interactionHint = \"Press E to open\";\n\n\tlet open = false;\n\tconst closedY = door.rotation.y;\n\tconst openY   = closedY + Math.PI / 2;\n\n\tdoor.on(\"interact\", () => {\n\t\topen = !open;\n\t\tRebur.Tween(door.rotation, { y: open ? openY : closedY }, ${2:0.4}, \"easeOutQuad\");\n\t\tdoor.interactionHint = open ? \"Press E to close\" : \"Press E to open\";\n\t\tRebur.Sound.play(open ? \"door_open\" : \"door_close\", { volume: 0.6 });\n\t});\n}",
    snippet: true,
  },
  {
    label: "snippet: Planet Gravity",
    kind: K.Snippet,
    detail: "Spherical gravity — walk on a planet surface",
    doc: "Attaches spherical gravity to a planet mesh and zeros global gravity so players orbit the sphere's surface.\n\nUses entity.gravity, Rebur.Physics.gravity.",
    insert: "// Planet Gravity — walk on a sphere\nconst planet = Rebur.Workspace.find(\"${1:Planet}\");\nif (planet) {\n\tplanet.gravity = { strength: ${2:20}, radius: ${3:50} };\n}\n\n// Zero global gravity — planet provides all pull\nRebur.Physics.gravity = 0;\n\nlog(\"Planet gravity active!\");",
    snippet: true,
  },
  {
    label: "snippet: NPC Patrol",
    kind: K.Snippet,
    detail: "Move an NPC between waypoints on each tick",
    doc: "Moves an entity between a list of world positions on every tick, rotating it to face its target.\n\nUses Rebur.on('tick'), entity.position, entity.rotation.",
    insert: "// NPC Patrol — move between waypoints\nconst npc = Rebur.Workspace.find(\"${1:Guard}\");\nconst waypoints = [\n\t{ x: ${2:-10}, y: 1, z: 0 },\n\t{ x: ${3:10},  y: 1, z: 0 },\n\t{ x: 0,       y: 1, z: ${4:10} },\n];\n\nlet waypointIndex = 0;\nconst SPEED = ${5:4};\n\nRebur.on(\"tick\", (dt) => {\n\tif (!npc || npc.destroyed) return;\n\n\tconst target = waypoints[waypointIndex];\n\tconst dx = target.x - npc.position.x;\n\tconst dz = target.z - npc.position.z;\n\tconst dist = Math.sqrt(dx * dx + dz * dz);\n\n\tif (dist < 0.3) {\n\t\twaypointIndex = (waypointIndex + 1) % waypoints.length;\n\t\treturn;\n\t}\n\n\tnpc.position = {\n\t\tx: npc.position.x + (dx / dist) * SPEED * dt,\n\t\ty: npc.position.y,\n\t\tz: npc.position.z + (dz / dist) * SPEED * dt,\n\t};\n\tnpc.rotation = { x: 0, y: Math.atan2(dx, dz), z: 0 };\n});",
    snippet: true,
  },
